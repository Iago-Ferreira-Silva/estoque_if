...
describe("Autorização — Produtos", () => {
  test("Agente PODE criar produto (201)", async () => {
  ...
  test("Coordenador PODE excluir produto (200)", async () => {
    const res = await request(app)
  ...
  test("Secretário PODE editar produto (200)", async () => {
    const res = await request(app)
      .put("/api/produtos/1")
...
describe("Autorização — Movimentações", () => {
  test("Agente PODE registrar movimentação (201)", async () => {
    const res = await request(app)
