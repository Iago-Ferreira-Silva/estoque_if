...
function renderTabela(lista) {
  ...
  tbody.innerHTML = lista
    .map((p) => {
      ...
      const qtdAtualPrinc = Number(
        p.qtd_atual / p.fator_conversao,
      ).toLocaleString("pt-BR");
      const qtdMinimaPrinc = Number(
        p.qtd_minima / p.fator_conversao,
      ).toLocaleString("pt-BR");
...
document
  ...
  .addEventListener("change", atualizarCamposConversao);
document
  .getElementById("fatorConversao")
  .addEventListener("input", atualizarCamposConversao);
document
  .getElementById("unidadeMinima")
  .addEventListener("input", atualizarCamposConversao);
...
function limparForm() {
  ...
  document.getElementById("fatorExemplo").textContent =
    "1 unidade = 1 unidade (sem conversão)";
  document.getElementById("conversaoAviso").hidden = true;
  document.getElementById("qtdAtualHint").textContent =
    "Informe a quantidade em unidades";
  document.getElementById("qtdMinimaHint").textContent =
    "Informe o mínimo em unidades";
...
document.getElementById("btnSalvar").addEventListener("click", async () => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      mostrarToast(data.message || "Erro ao salvar produto.", "error");
      return;
    }
...
document
  .getElementById("confirmacaoClose")
  .addEventListener("click", fecharConfirmacao);
document
  .getElementById("confirmacaoCancelar")
  .addEventListener("click", fecharConfirmacao);
...
confirmacaoConfirmar.addEventListener("click", async () => {
  ...
  try {
    const response = await fetch(
      `http://localhost:3000/api/produtos/${idParaExcluir}`,
      {
        method: "DELETE",
        headers: getHeaders(),
      },
