...
async function carregarDados() {
  try {
    const [resMov, resProd, resSeto] = await Promise.all([
      fetch("http://localhost:3000/api/movimentacoes", {
        headers: getHeaders(),
      }),
...
document.getElementById("produtoMov").addEventListener("change", function () {
  ...
  document.getElementById("infoMinima").textContent = p.unidade_minima;
...
function atualizarPreviewConversao() {
  ...
  const fatorOpcao = Number(
    unidadeSelect.selectedOptions[0]?.dataset.fator || 1,
  );
  const preview = document.getElementById("conversaoPreview");
  ...
  if (fatorOpcao === 1) {
    textoPreview.textContent = `${quantidade} ${unidadeEscolhida} serão registrados diretamente no estoque.`;
  } else {
...
document
  ...
  .addEventListener("input", atualizarPreviewConversao);
document
  .getElementById("unidadeMov")
  .addEventListener("change", atualizarPreviewConversao);
...
document.getElementById("btnSalvar").addEventListener("click", async () => {
  ...
  if (
    tipo === "saida" &&
    quantidadeConvertida > Number(produtoSelecionado.qtd_atual)
  ) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      mostrarToast(data.message || "Erro ao registrar movimentação.", "error");
      return;
    }
